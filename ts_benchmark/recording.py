...
def save_log(
    ...
    # Prepend dataset name to the filename when the record covers a single dataset,
    # so that "CATCH.1234.xxx.csv.tar.gz" becomes "PSM.CATCH.1234.xxx.csv.tar.gz".
    dataset_prefix = ""
    if "file_name" in result_df.columns:
        datasets = result_df["file_name"].dropna().unique()
        if len(datasets) == 1:
            dataset_prefix = os.path.splitext(str(datasets[0]))[0] + "."

    record_filename = dataset_prefix + file_prefix + get_unique_file_suffix()
